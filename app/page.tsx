...
export default function Home() {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      ...
      // 🔥 Arreglamos images aquí
      const parsedProperties = properties.map((p) => ({
        ...p,
        images: typeof p.images === "string"
          ? p.images.replace("{", "").replace("}", "").split(",")
          : p.images,
      }));
        ...
        <div className="container px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {featured.map((property) => {
              console.log("FEATURED:", property.images);
              return (
                <PropertyCard
                  key={property.id}
                  id={property.id}
        ...
        <div className="container px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {newProperties.map((property) => {
              console.log("NEW LISTINGS:", property.images);
              return (
                <PropertyCard
                  key={property.id}
                  id={property.id}
        ...
        <div className="container px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {rentals.map((property) => {
              console.log("RENTALS:", property.images);
              return (
                <PropertyCard
                  key={property.id}
                  id={property.id}
